...
export function ProductListItem({ product }: ProductListItemProps) {
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-4 p-4 border rounded-lg transition-all duration-300 group bg-gradient-to-r from-background to-secondary/5 hover:shadow-lg hover:shadow-secondary/20 hover:translate-y-[-2px]">
      {/* Product Image - Fixed dimensions with proper positioning */}
      <div className="relative w-full sm:w-48 h-48 flex-shrink-0">
        <Link href={`/products/${product.id}`} className="block h-full">
          <div className="relative h-full w-full overflow-hidden bg-secondary/5 rounded-md">
