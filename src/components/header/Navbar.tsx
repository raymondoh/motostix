...
//   useEffect(() => {
//     if (isUpdatingSessionRef.current || (pathname === lastPathnameRef.current && hasCheckedSessionRef.current)) return;
//     const now = Date.now();
//     if (now - lastSessionCheckTimeRef.current < 2000) return;
...
//       update().finally(() => (isUpdatingSessionRef.current = false));
//     }
//   }, [pathname, update]);

//   useEffect(() => {
//     const handleStorageChange = (event: StorageEvent) => {
//       if (event.key?.includes("session") || event.key?.includes("token")) update();
//     };
